...
layui.use(['form', 'layer', 'authtree'], function () {
    ...
    form.on("submit(addRole)", function (data) {
        ...
        $.ajax({
            ...
            data: {
                ...
                MenuIds: authtree.getChecked('#dotnet-auth-tree'),
                Remark: $(".Remark").val()  //用户简介
            },
            dataType: "json",
            headers: {
                "X-CSRF-TOKEN-dotnet": $("input[name='AntiforgeryKey_dotnet']").val()
            },
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            authtree.render('#dotnet-auth-tree', trees, {
                inputname: 'ids[]'
                , layfilter: 'dotnet-check-auth'
                , autowidth: true
            });

            authtree.on('change(dotnet-check-auth)', function (data) {
                console.log('监听 authtree 触发事件数据', data);
            });
            authtree.on('dblclick(dotnet-check-auth)', function (data) {
