    ...
    public class ManagerService : IManagerService
    {
        ...
        public async Task<BaseResult> AddOrModifyAsync(ManagerAddOrModifyModel item)
        {
            ...
            if (item.Id == 0)
            {
                ...
                manager.Password = AESEncryptHelper.Encode(DotnetCmsKeys.DefaultPassword, DotnetCmsKeys.AesEncryptKeys);
        ...
        {
            model.Password = AESEncryptHelper.Encode(model.Password.Trim(), DotnetCmsKeys.AesEncryptKeys);
            ...
            if (manager != null)
            {
                ...
                {
                    ActionType = DotnetCmsEnums.ActionEnum.SignIn.ToString(),
        ...
        public async Task<BaseResult> ChangePasswordAsync(ChangePasswordModel model)
        {
            ...
            if (oldPwd == AESEncryptHelper.Encode(model.OldPassword, DotnetCmsKeys.AesEncryptKeys))
            {
                var count = await _repository.ChangePasswordByIdAsync(model.Id, AESEncryptHelper.Encode(model.NewPassword.Trim(), DotnetCmsKeys.AesEncryptKeys));
                if (count > 0)
                {
                    result.ResultCode = ResultCodeAddMsgKeys.CommonObjectSuccessCode;
                    result.ResultMsg = ResultCodeAddMsgKeys.CommonObjectSuccessMsg;
